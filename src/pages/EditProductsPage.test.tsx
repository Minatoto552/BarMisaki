...
describe('商品編集', () => {
  it('画像を選び直さず名前とレシピを保存する', async () => {
    open(); fireEvent.click(screen.getByRole('button', { name: '星空を編集' }));
  ...
  it('削除の確認でキャンセルでき、明示的な確定後だけ削除する', async () => {
    open(); fireEvent.click(screen.getByRole('button', { name: '星空を削除' }));
    expect(state.deleteProduct).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole('button', { name: 'キャンセル' }));
    expect(state.deleteProduct).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole('button', { name: '星空を削除' }));
  ...
  it('未登録ユーザーの変更を禁止し、商品検索できる', () => {
    state.profile = null;
    open(); expect(screen.getByRole('button', { name: '星空を編集' })).toBeDisabled();
    expect(screen.getByRole('button', { name: '星空を削除' })).toBeDisabled();
